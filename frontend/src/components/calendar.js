    ...
    useEffect(()=>{
        const fetchCalendarTrades = async () => {
            try {
                ...
                setCurrentCalendarData(await res.json())
            } catch (error) {
                console.error(error)
            }
        }
            ...
            <div className="h-full w-full bg-white shadow-2xl flex flex-col rounded-2xl overflow-hidden">
                <div className="grid grid-cols-2 w-full px-5 h-20 bg-gray-800 py-5">
                    <div className="flex flex-col place-content-center">
                        <div className="text-lg font-bold text-white">Trading Calendar</div>
                        <div className="text-xs px-1 text-slate-300">View Trades by day</div>
                    </div>
                        ...
                        days_of_month.map((day, index)=>(
                            ...
                            : ( !tradesByDay.get(day)
                                ...
                                : (    
                                    <div key={index} onClick={()=>setSelectedField(day)} className={`flex flex-col text-slate-600 h-full w-full border-2 rounded-xl p-4 aspect-square
                                     hover:cursor-pointer hover:shadow-md hover:scale-105 transition-all duration-300 
                                     ${tradesByDay.get(day).outcome == 'Win' ? "bg-green-200 border-green-300" : "bg-red-200 border-red-300"}
                                    ...
                                    `}>
                                        ...
                                        <div className={`flex flex-col h-full text-xs`}>
                                            <div className={`${tradesByDay.get(day).outcome == 'Win' ? "text-green-600" : "text-red-600"}`}>{`${tradesByDay.get(day).outcome == 'Win' ? "+" : "-"} ${Math.floor(tradesByDay.get(day).pnl)}$`}</div>
                                            <div className="text-slate-500">1 trade</div>
                                        </div>
                                    </div>
                                )    
